...
кроме переметра:
- *-a cat_id* (=<int>) принимете только один id категории

#### # scrapy crawl oz_qty
собирает информацию об остатках.
параметры также как в *scrapy crawl wb_cost*
дополнительные параметры:
Информация об использовании в разделе Сервис Selenoid-ui
...
параметры в настройках:
  - OZ_QTY_PORTION определяет порцию. количество товаров которые добавляются в корзину.
  - OZ_QTY_COUNT_PORTION количество порций для одной сессии.
  - USE_PROXY_FOR_OZ_QTY Отключает/включает внутренний прокси.

Принцип работы парсера oz_qty в следующем.
Открывается браузерная сессия в Selenoid. Скрипт добавляет порцию (OZ_QTY_PORTION) товаров в корзину ozon.ru, переходит туда и считывает данные об остатках. Удаляет товары из корзины. Повторяет цикл добавления/удаления OZ_QTY_COUNT_PORTION число раз.
Больше всего времени уходит на открытие новой сессии, но это единственный способ использовать прокси ротатор. Если использовать прокси на уровне шлюза (например через vpn), а внутренний отключить, то можно увеличить число count_portion, или поставить 0 (что означает по выбору программы).
Производительность можно увеличить за счет параметра AUTOTHROTTLE_TARGET_CONCURRENCY. Но Важно следить, что бы Это значение не превышало допустимую квоту Selenoid-ui (возможное количество одновременных сессий).
Посмотреть за работой сессий можно по адресу http://127.0.0.1:8080. Также, если озон выбросил капчу, ее можно решить там в ручную через виртуальный браузер.
